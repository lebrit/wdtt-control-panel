...
Вкладка **Xray** позволяет установить `wgcf`, создать Cloudflare WARP-профиль и автоматически добавить исходящий `warp` в Managed-конфигурацию Xray. Кнопка **Пинг WARP** запускает изолированную временную проверку через этот профиль, показывает задержку, IP выхода и подтверждение Cloudflare `warp=on`. WARP остаётся обычным исходящим: его можно выбрать в правилах маршрутизации Xray.
