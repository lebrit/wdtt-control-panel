...
Чтобы на роутере не устанавливать отдельный WDTT-клиент, панель включает на сервере Xray VLESS over WebSocket через HTTPS панели. Podkop Plus импортирует это как обычную sing-box/VLESS-подписку. Скрипт настройки только записывает URL подписки в уже установленный Podkop/Podkop Plus, добавляет автообновление и переключает секцию `podkop.main` на `connection_type='proxy'` / `proxy_config_type='subscription'`.
