...
**Текущая версия: 0.10.14**

Отдельная web-панель для [amurcanov/proxy-turn-vk-android](https://github.com/amurcanov/proxy-turn-vk-android). При установке и обновлении она автоматически собирает совместимое расширение WDTT с резервной копией прежнего бинарника: метки Telegram и учёт трафика всех пользователей работают без отдельного действия. Если сборка временно не удалась, система повторяет её автоматически.
...
## Подключение к WDTT Fleet Manager

Начиная с `0.10.12`, панель содержит выключенный по умолчанию исходящий **Fleet Agent**. Вкладка **Fleet** принимает только HTTPS-адрес агента и одноразовый грант, созданный в центральной панели. После сохранения агент сам регистрирует этот узел, отправляет heartbeat и снимки WDTT-пользователей, а затем забирает лишь адресованные ему команды. В `0.10.13` обновление автоматически перезапускает включённый агент.

Если в Fleet остаётся ошибка, запустите на сервере именно с WDTT Control Panel безопасную диагностику и передайте полученный отчёт: он не выводит токены, гранты, пароли или список пользователей.

```bash
curl -fsSL https://raw.githubusercontent.com/lebrit/wdtt-control-panel/main/fleet-diagnostics.sh | sudo bash
```

Агент вызывает фиксированный локальный список операций: создать, изменить, удалить или прочитать WDTT-пользователя и получить снимок узла. В центр не передаются private keys, `passwords.json`, токены, Xray/WARP-настройки, журналы и команды shell. Агент не открывает входящий административный порт.

Перед подключением реальных узлов рекомендуется включить mTLS на Fleet Manager. Текущая версия использует одноразовый грант и временный токен узла для первого совместимого протокола.
...
Для автоматизации тот же скрипт принимает `install`, `update`, `rollback`, `status`, `renew-cert`, `change-password`, `uninstall` и параметры `--domain`, `--ip`, `--user`, `--password`, `--email`, `--https-port`, `--path`, `--wdtt`, `--wdtt-password`, `--version`, `--non-interactive`.
