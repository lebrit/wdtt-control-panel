...
**Текущая версия: 0.11.25**

Отдельная web-панель для [SpaceNeuroX/proxy-turn-vk-android](https://github.com/SpaceNeuroX/proxy-turn-vk-android). При установке и обновлении она автоматически собирает совместимое расширение WDTT с резервной копией прежнего бинарника; по умолчанию используется серверная часть релиза `v1.4.0`. Метки Telegram и учёт трафика всех пользователей работают без отдельного действия. Если сборка временно не удалась, система повторяет её автоматически.
