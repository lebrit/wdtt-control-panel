...
- OpenWrt/Podkop Plus-подписка для пользователя без дополнительного клиента на роутере: персональный token URL, native VLESS/WebSocket/TLS JSON, VLESS URL, qWDTT JSON и скрипт настройки Podkop Plus;
...
В меню действий пользователя есть пункт **OpenWrt**. Он выдаёт персональный token URL с несколькими форматами: `podkop-plus.json`, `vless.txt`, `qwdtt.json` и `install.sh`. Ссылка перестаёт работать, если пользователь удалён, отключён или истёк.

Чтобы на роутере не устанавливать отдельный WDTT-клиент, панель включает на сервере Xray VLESS over WebSocket через HTTPS панели. Podkop Plus импортирует это как обычную sing-box/VLESS-подписку. Скрипт настройки только записывает URL подписки в уже установленный Podkop Plus, добавляет автообновление и создаёт/обновляет секцию `wdtt` в `/etc/config/podkop-plus`. Для старого Podkop оставлен совместимый fallback.
