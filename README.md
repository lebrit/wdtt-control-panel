...
Отдельная web-панель для [SpaceNeuroX/proxy-turn-vk-android](https://github.com/SpaceNeuroX/proxy-turn-vk-android). При установке и обновлении она автоматически собирает совместимое расширение WDTT с резервной копией прежнего бинарника; по умолчанию используется серверная часть релиза `v1.3.5` с поддержкой режима маскировки «Видео». Метки Telegram и учёт трафика всех пользователей работают без отдельного действия. Если сборка временно не удалась, система повторяет её автоматически.
