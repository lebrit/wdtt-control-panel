...
- OpenWrt/Podkop Plus-подписка для пользователя: персональный token URL, metadata JSON, `wdtt://`, qWDTT JSON и install-скрипт для роутера;
...
В меню действий пользователя есть пункт **OpenWrt**. Он выдаёт персональный token URL с несколькими форматами: `podkop-plus.json`, `wdtt.txt`, `qwdtt.json` и `install.sh`. Ссылка перестаёт работать, если пользователь удалён, отключён или истёк.

WDTT-пользователь не является обычным VLESS/sing-box outbound. Для Podkop Plus используется режим VPN-интерфейса: на роутере должен быть совместимый WDTT-клиент, который поднимает интерфейс `wdtt0`, после чего Podkop Plus маршрутизирует выбранные списки через этот интерфейс. Скрипт установки сохраняет подписку, добавляет автообновление и переключает секцию `podkop.main` на `connection_type='vpn'` / `interface='wdtt0'`, если интерфейс уже существует.
